		...
		if(Ins.EffectiveI>0)
		{
			if(Ins.EffectiveI==Ins.PhCurrent)
			{
				if(isPulseOn==0)
				{
					...
					#if (defined(IS_ADD_NM_CURRENTS) && (IS_ADD_NM_CURRENTS == 1))
						secondChCurrent=Ins.NeuCurrent;
						doDeltaSigmaADC_NM(CH_I2,NM_SHORT_SAMPLE);
						if((secondChCurrent==0)&&Ins.NeuCurrent)
			...
			else if(Ins.EffectiveI==Ins.NeuCurrent)
			{
				if(isPulseOn==0)
				{
					doDeltaSigmaADC_NM(CH_I2,NM_SHORT_SAMPLE);
					#if (defined(IS_ADD_NM_CURRENTS) && (IS_ADD_NM_CURRENTS == 1))
						secondChCurrent=Ins.PhCurrent;
						doDeltaSigmaADC_NM(CH_I1,NM_SHORT_SAMPLE);
						if((secondChCurrent==0)&&Ins.PhCurrent)
							doDeltaSigmaADC_NM(CH_I1,NM_LONG_SAMPLE);
		...
		else 
		{
			...
			if(Ins.EffectiveI>0)
			{
				...
				else
				{
					...
					#if (defined(IS_ADD_NM_CURRENTS) && (IS_ADD_NM_CURRENTS == 0))	
...
void doDeltaSigmaADC_NM(uint8_t chOption,uint16_t no_of_samples)
{
		...
		if(Ins.EffectiveI==0)
		{
			if(NM_SHORT_SAMPLE==wait)
			{
				...
				if(Ins.PhCurrent<IMAX_CURRENT_NM)
				{
					...
					{
						if((Ins.PhCurrent>NM_MIN_PH_CURRENT_THRESHOLD)&&(Ins.PhCurrent+Ins.NeuCurrent>NM_CURRENT_THRESHOLD))
						{
		...
		if(Ins.EffectiveI==0)
		{
			if(NM_SHORT_SAMPLE==wait)
			{
				...
				if(Ins.NeuCurrent<IMAX_CURRENT_NM)
				{
					...
					{
						if((Ins.NeuCurrent>NM_MIN_NU_CURRENT_THRESHOLD)&&(Ins.PhCurrent+Ins.NeuCurrent>NM_CURRENT_THRESHOLD))
				//	if(Ins.NeuCurrent>NM_MIN_NU_CURRENT_THRESHOLD)
						{
	...
	if((Ins.PhCurrent<NM_MIN_PH_CURRENT_THRESHOLD))				
		Ins.PhCurrent=0;
	if((Ins.NeuCurrent<NM_MIN_NU_CURRENT_THRESHOLD))		
		Ins.NeuCurrent=0;
